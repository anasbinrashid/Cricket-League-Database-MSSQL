...
insert into performance values (2, 2, 3, 28, 4, 6)
...
select top (1) player.playername as 'Player Name', performance.catches as 'Catches'
from performance
inner join player
...
select team.teamname as 'Team Name', team.homestadium as 'Home Stadium'
from team

-- Query 4 : Show the total capacity of a specific stadium.

select stadium.stadiumname as 'Stadium Name', stadium.capacity as 'Capacity'
from stadium
where stadiumname = 'Rawalpindi Cricket Stadium'

-- Query 5 : Calculate the average runs scored by players in each team.

select player.teamname as 'Team Name', avg (performance.runsscored) as 'Average Runs'
from player 
...
select stadium.stadiumname  as 'Stadium Name', count (match.stadiumid) as 'Total Matches'
from stadium 
inner join match 
...
select player.playerid as 'Player ID', player.playername as 'Player Name', performance.runsscored as 'Runs Scored'
from player 
inner join performance 
on player.playerid=performance.playerid
...
select team.teamname as 'Team Name', team.numberofwins as 'Wins'
from team
where numberofwins > 3

-- Query 9 : Find all players who are bowlers in any team.

select player.playerid as 'Player ID', player.playername as 'Player Name', player.playerrole as 'Player Role', player.teamname as 'Player Team'
from player
where playerrole = 'Bowler'

-- Query 10 : List players who scored more than 50 runs and took at least 3 wickets in a single match.

select player.playerid as 'Player ID', player.playername as 'Player Name', player.teamname as 'Player Team', performance.runsscored as 'Runs Scored', performance.wicketstaken as 'Wickets Taken'
from player 
inner join performance
on player.playerid = performance.playerid
...
select top (1) player.teamname as 'Team Name', sum (performance.runsscored) as 'Total Runs'
from player 
join performance
on player.playerid = performance.playerid
group by teamname
...
select match.matchid as 'Match ID', match.matchtype as 'Match Type', match.matchdate as 'Match Date', t1.teamname as Team1, t2.teamname as Team2, winner.winningteam as 'Winner'
from match 
join team t1 
...
select distinct match.matchtype as 'Match Type'
from match
...
select top (3) player.playerid as 'Player ID', player.playername as 'Player Name', player.teamname as 'Player Team', match.matchtype as 'Match Type', performance.runsscored as 'Final Match Score'
from performance 
inner join player 
on performance.playerid = player.playerid
inner join match 
...
select winner.winningteam as 'Winner', player.playername as 'Player Name', player.teamname as 'Player Team', performance.matchid as 'Match ID', performance.runsscored as 'Runs Scored'
from performance
inner join winner
on performance.matchid = winner.matchid
inner join player
on player.playerid = performance.playerid 
where performance.runsscored > 50 and winner.winningteam = player.teamname
...
select top (3) player.playername as 'Player Name', avg (performance.runsscored) as AverageRuns, match.matchtype as 'Match Type'
from performance 
inner join player 
on performance.playerid = player.playerid
inner join match 
...
select team.teamname as 'Team Name', stadium.stadiumname as 'Stadium Name', avg (performance.runsscored) as AverageRunsScored, avg (performance.wicketstaken) as AverageWicketsTaken
from match
inner join performance
...
select team.teamname as 'Team Name', stadium.stadiumname as 'Stadium Name', count (winner.winningteam) as Wins
from winner
inner join match 
...
select player.playerid as 'Player ID', player.playername as 'Player Name', player.teamname as 'Team Name', winner.winningteam as 'Winner', performance.runsscored as 'Runs Scored', performance.wicketstaken as 'Wickets Taken', performance.catches as 'Catches Taken'
from performance
inner join match
on performance.matchid = match.matchid
inner join winner
...
-- Importance: Correlated subqueries allow you to reference values from the outer query within the inner query and are crucial for scenarios where you need to perform comparisons based on conditions from the outer query.
-- Usage: Correlated subqueries can be used in various scenarios such as filtering based on related rows, performing row-wise computations, and optimizing complex queries.

select performance.playerid as 'Player ID', player.playername as 'Player Name', performance.matchid as 'Match ID', performance.wicketstaken as 'Wickets Taken' 
from performance
inner join player
on player.playerid = performance.playerid
where performance.wicketstaken > (	select p.wicketstaken
									from performance p
									where p.playerid = 7  )

--> Union

-- Importance: The Union operator is essential for combining the results of multiple queries into a single result set as it eliminates duplicate rows by default, which can be beneficial in various data consolidation scenarios.
-- Usage: It is commonly used to combine data from different sources, and retrieve distinct values from multiple queries.
...
select match.matchtype as MatchType, match.matchid as MatchID
from match
...
-- Importance: The Group By clause is crucial for performing aggregate functions like Count, Sum, Avg, on groups of rows and it allows you to summarize data and obtain insights from large datasets.
-- Usage: It is extensively used in reporting, analytics, and data summarization tasks where you need to analyze data based on certain categories or groups.

select playerrole as 'Player Role', count (*) as 'Total Players'
from player
group by playerrole
...
-- Importance: The Like operator with wildcard characters allows you to perform substring comparisons in SQL and it is useful for pattern matching and searching for specific patterns within strings.
-- Usage: It can be used for various purposes such as searching for names starting with a certain letter or containing specific characters, filtering based on patterns in textual data.
...
-- Importance: The Having clause is used in combination with the Group By clause to filter groups based on aggregate conditions as it enables you to filter aggregated data according to specific criteria.
-- Usage: It is helpful when you need to filter groups based on aggregated values such as filtering groups with a certain count, sum, or average and it's commonly used in reporting and analysis queries.

select playerrole  as 'Player Role', count (*) as 'Total Players'
from player
group by player.playerrole
having player.playerrole in ('All-Rounder', 'Bowler')
